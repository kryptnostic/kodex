...
public class UserKey implements Principal,Serializable {
    ...
    public static boolean isValidUsername( String name ) {
        Preconditions.checkState( StringUtils.isAlphanumeric( name ) , "Realm name cannot be blank and must consist of only alpha numeric characters." );
        Preconditions.checkState( name.length() < MAX_USERNAME_LENGTH , "Username cannot be longer than " + MAX_USERNAME_LENGTH + "characters" );
        return true;
    }

    public static boolean isValidRealm( String realm ) {
        Preconditions.checkState( StringUtils.isAlphanumeric( realm ) , "Realm name cannot be blank and must consist of only alpha numeric characters." );
        Preconditions.checkState( realm.length() < MAX_REALM_LENGTH , "Realm name cannot be longer than " + MAX_REALM_LENGTH + "characters" );
        return false;
    }
